...
app = FastAPI(
    ...
    description="社区助老服务闭环督办与风险升级系统：支持服务类型级SLA配置、社区级节假日/工作时段扣除规则、重复诉求合并建议、督办记录、人工确认升级、跟进计划和回访结果归档；系统可按同一老人、同类服务、近7/15/30天窗口识别疑似重复诉求并生成可确认的合并建议；对超时未闭环工单按风险等级、超时时长、历史未完成次数自动计算督办优先级，并生成下一步跟进建议；新增助老服务资源调度与供需匹配能力；新增服务质量评价与异常预警闭环能力，支持评价模板配置、多维度满意度评价、异常自动检测、整改任务流转和多维度质量统计分析。",
    version="3.0.0"
)
